...
24. **Stateful `scry.tcp.connect` userdata + `scry.udp.send`.** The Lua API gained `tcp.connect` (multi-round-trip userdata with `:send`/`:read`/`:close`) and `udp.send` (one-shot with optional reply). Both follow the existing timeout convention: read timeouts surface as `('', 'timeout')` so scripts can distinguish "no reply yet" from hard failures. Closes §10 #9 (UDP deferred) and unblocks protocol scripts that can't be expressed as a single request/response — `smb-version.lua` ships as the first user. _Decided 2026-04-24, scripting-polish sweep._
25. **Viper-backed config file.** §10 #6 superseded. CLI reads `~/.config/scry/config.yaml` (or `%APPDATA%\scry\config.yaml` on Windows, or `$SCRY_CONFIG`, or `--config FILE`). Precedence: CLI flag > `SCRY_*` env > config file > flag default. Only flags the user didn't explicitly set get pulled from config. Explicit paths must exist; implicit defaults silently skip when missing. Implementation in `internal/cli/config.go`. _Decided 2026-04-24, config-docs-live sweep._
26. **Docs site: GitHub Pages via Jekyll, not mdBook/MkDocs.** Pages uses the `pages-themes/cayman` remote theme plus `actions/jekyll-build-pages` — zero extra toolchain in CI, no Rust/Node required. Content lives under `docs/site/` as plain Markdown; `.github/workflows/pages.yml` builds and deploys on pushes to `main` that touch that directory. Seven pages in the initial cut (index, install, usage, output, scripting, syn, config). _Decided 2026-04-24, config-docs-live sweep._
27. **`--live` in-place table, ANSI-only (no bubbletea).** Plan §4.6 calls for a "live-updating table" and §2 tagged bubbletea as "stretch." We ship an opt-in `--live` flag that redraws a single host-summary table via bare ANSI cursor-up + clear, rendered by lipgloss. No new dependency. Non-TTY stdout downgrades to `FormatHuman` with a stderr warning so pipelines stay clean. `internal/output/live.go`. Bubbletea-style full TUI remains a future option if the ANSI path proves limiting. _Decided 2026-04-24, config-docs-live sweep._
28. **NSE compatibility shim: Tier-1 only.** §10 #3 revised. `nmap.new_socket` + `stdnse.get_script_args` + `stdnse.print_debug` ship in `internal/script/api_nse_shim.go` — enough to run the majority of small NSE scripts that only touch socket primitives. Tier-2 (`shortport.*`, `creds.*`, `brute.*`, protocol libs like `http`/`vulns`/`smb`) is intentionally absent; scripts that need them fail with a standard Lua missing-global error. Rationale: Tier-1 is a small, stable surface; Tier-2 would pull in nmap-library semantics with no feasible stopping point. Compatibility matrix in `scripts/README.md`. _Decided 2026-04-24, final-deferred sweep._
29. **Windows Npcap SYN scaffold, not verified.** `internal/portscan/syn_windows.go` mirrors the Linux implementation with an Npcap-aware `setupSYN`/`pickInterface` and a broadcast DST-MAC fallback. Build-verified via cross-compile; end-to-end verification still requires a Windows host with elevated Npcap. Duplicated the scanner machinery rather than refactoring behind a platform interface — lower risk while the Linux path is the known-good reference; dedup appropriate after a Windows contributor confirms the flow. The `GetIpNetTable2` ARP path is the documented next step and tracked in DEFERRED.md. _Decided 2026-04-24, final-deferred sweep._
30. **Pin `GOTOOLCHAIN=go1.23.0` in Makefile.** `go get <pkg>@latest` was repeatedly bumping `go.mod`'s go directive up to whatever toolchain was installed locally (observed up to 1.25). Pinning `GOTOOLCHAIN` stops the silent drift. Developers who intentionally want a newer toolchain can set `GOTOOLCHAIN=local`. _Decided 2026-04-24, final-deferred sweep._
